...
(function() {
    ...
    interface BlazorDevTools {
        _dotNetRef?: DotNetObjectReference;
        _initialized?: boolean;
        initialize?: (ref: DotNetObjectReference) => void;
    ...
    const componentTree = new Map<number, ComponentNode>();
    ...
    function initialize(): void {
        ...
        const bdt = (window as any).blazorDevTools as BlazorDevTools | undefined;
        if (bdt?._initialized && bdt._dotNetRef) {
            onReady(bdt._dotNetRef);
        }

        // Listen for ready event from NuGet bootstrap
        window.addEventListener('blazorDevToolsReady', ((event: CustomEvent) => {
            onReady(event.detail);
        }) as EventListener);

        // Listen for lifecycle events
        window.addEventListener('blazorDevToolsEvent', ((event: CustomEvent) => {
    ...
    function onReady(ref: DotNetObjectReference): void {
        dotNetRef = ref;
        isReady = true;
        ...
        // Fetch circuit ID
        dotNetRef.invokeMethodAsync<string>('GetCircuitId').then(id => {
    ...
    async function getAllComponents(): Promise<unknown[]> {
        if (!dotNetRef) throw new Error('Not connected to .NET registry');
        return await dotNetRef.invokeMethodAsync<unknown[]>('GetAllComponentsDto');
    }

    async function getComponent(componentId: number): Promise<unknown | null> {
        if (!dotNetRef) throw new Error('Not connected to .NET registry');
        return await dotNetRef.invokeMethodAsync<unknown>('GetComponentInfo', componentId);
    }

    async function getCircuitId(): Promise<string | null> {
        if (!dotNetRef) throw new Error('Not connected to .NET registry');
        return await dotNetRef.invokeMethodAsync<string>('GetCircuitId');
    }

    async function getComponentCount(): Promise<number> {
        if (!dotNetRef) throw new Error('Not connected to .NET registry');
        return await dotNetRef.invokeMethodAsync<number>('GetComponentCount');
    }
    ...
    function tryHookPillar3(): boolean {
        if (pillar3Hooked) return true;
        ...
        if (Blazor._internal.renderBatch && typeof Blazor._internal.renderBatch === 'function') {
            ...
            Blazor._internal.renderBatch = function(rendererId: number, batch: unknown) {
                try {
        ...
        if ((window as any).WebAssembly && Blazor.platform) {
        ...
        if (Blazor._internal.attachRootComponentToElement && !hooked) {
            ...
            Blazor._internal.attachRootComponentToElement = function(
                selector: string, 
                componentId: number, 
                rendererId: number
            ) {
        ...
        if (!hooked && typeof Blazor.start === 'function' && !(Blazor as any)._started) {
            const originalStart = Blazor.start;
            Blazor.start = async function(...args: unknown[]) {
    ...
    function trackRootComponent(componentId: number): void {
    ...
    function processRenderBatch(batch: any): void {
    ...
    function processReferenceFrames(frames: any, updatedIds: number[]): void {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (frameType === FrameType.Component) {
                    ...
                    if (frame.componentType) {
                        ...
                        if (node) node.typeName = frame.componentType;
                    }
                }
            }
        } catch (e) {
    ...
    function trackComponentRender(componentId: number): void {
    ...
    function trackComponentDisposal(componentId: number): void {
    ...
    function setParentChild(parentId: number, childId: number): void {
    ...
    async function sendUpdatesToRegistry(updatedIds: number[], disposedIds: number[]): Promise<void> {
        if (!dotNetRef) return;
    ...
    function postToContentScript(type: string, data: unknown): void {
