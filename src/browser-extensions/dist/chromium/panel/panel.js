...
function renderRankedView() {
    ...
    container.querySelectorAll('.ranked-row').forEach(row => {
        row.addEventListener('click', () => {
            ...
            const componentEvent = events.find(e => e.componentName === componentName && e.eventType === 'BuildRenderTree');
            if (componentEvent) {
...
function renderFlamegraph() {
    ...
    const renderEvents = events.filter(e => e.componentId >= 0 && (e.eventType === 'BuildRenderTree' ||
...
function renderSwimlaneInstance(componentId, componentName, visibleStart, visibleRange, renderEvents) {
...
function generateTimeAxis(minTime, maxTime) {
